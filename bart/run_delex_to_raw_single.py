...
"""
Fine-tuning BART for language modeling on SLU data.
"""
...
from BartExt import BartForSlotFiling, BartForSlotFilingCRF, BartForGeneration
...
SLOT_PADDING_IDX = -100

# get splited label for each subword
# return token labels and subword labels(subword with mask 1)
...
class LineByLineTextDataset(Dataset):
    def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path: str, label_path: str, delex_path: str, block_size=512):
    ...
    def get_train_data(self, labels, delexs, examples, args):
    ...
    def get_value_mask(self, examples, values):
        value_mask = []
...
def mask_tokens(inputs, tokenizer, args, slot_ids, cont_ids):
...
def add_vocab(org_file, new_file, args):
